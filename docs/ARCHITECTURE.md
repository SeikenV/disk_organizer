...
- **语言选项 `--language <code|name>`**：分类按原有（已调优的）提示词产出后，再跑一轮
  **批量翻译**把每个 item 的 `category`/`purpose` 译成目标语言；最终报告直接用目标语言生成。
  ...
  - **按字形跳过**（`needs_translation`）：已经是目标字形的文本不再翻译，避免把正确的
    英文目录文案又翻回中文，也减少调用。
  - **两遍 + 措辞轮换**：第一遍每 ~10 个 item 批量翻译；第二遍对仍残留源语言的"漏网"
    条目**逐项重试并轮换措辞**（translate/transcribe/reiterate/rewrite — 小模型对动词敏感，
    换个说法常能成功）。从原文重试，避免在坏结果上二次翻译。
  - ✅ **实测**：默认 Qwen3.5-0.8B 上 `--language en` 经轮换后达到 0 残留（33 项全部英文）；
    代价是漏网项的逐项重试更慢。更大的文本模型（`--llm-model-path`）更快、漏网更少。
    （真实文件名等专有名词有意保留不译。）
